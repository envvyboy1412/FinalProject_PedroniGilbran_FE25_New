...
export function Navbar() {
  ...
  const handleLogout = async () => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/logout`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          apiKey: process.env.NEXT_PUBLIC_API_KEY as string,
    ...
    <nav className="bg-[#7D8D86] relative z-50">
      ...
      <div className="flex  items-center justify-between max-w-6xl mx-auto p-6">
        ...
        <div className="hidden md:flex items-center gap-6 relative">
          {!isAdmin && (
            <Link href="/user/cart" className="text-white text-2xl">
              🛒
            </Link>
          )}
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#ACBAC4] border-t">
          <div className="flex flex-col gap-4 p-6 text-white font-semibold">
            ...
            {!isAdmin && (
              <Link href="/user/cart" onClick={() => setMenuOpen(false)}>
                Cart
              </Link>
            )}
            ...
            {!isAdmin && (
              <Link href="/user/edit-profile" onClick={() => setMenuOpen(false)}>
                Edit Profile
              </Link>
            )}
